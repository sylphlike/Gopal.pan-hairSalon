...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 产品中心 <span class="c-gray en">&gt;</span> 入库管理</nav>
<div class="pd-20">
	<form id ="generalFormId" action="${ctx }/inProduct/inPriductList.action" method="POST">
		<table>
			<tr>
				<td>产品品牌</td>
				...
				<td>入库时间段</td>
				<td>
					<input type="text" class="input-text" style="width:150px" placeholder="输入入库起始时间"  name="startTime" value="${generalAttrName.startTime }"
					 onfocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'1990-03-08 11:30:00'})">&nbsp;-至-&nbsp;
					<input type="text" class="input-text" style="width:150px" placeholder="输入入库束时间"  name="endTime" value="${generalAttrName.endTime }"
					 onfocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'1990-03-08 11:30:00'})">
				</td>
			</tr>
		</table>
	
		<div class="text-c" style="margin-left:600px; margin-top:5px;margin-bottom:5px;" > 
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20" style="margin-top:5px;" > 
			<span class="l">
				...
				<a href="javascript:;" onclick="general_open_window_no_param('入库产品 ','${ctx}/inProduct/gotoInProduct.action','','510')" class="btn btn-primary radius">
					<i class="Hui-iconfont">&#xe600;</i> 入库产品 
				</a>
			</span>
		...
		<div class="mt-20" style="margin-top:5px;">
			<table class="table table-border table-bordered table-hover table-bg ">
				...
				<tbody>
					<c:forEach items="${generalAttrName.recoredList }" var="storeIn">
						<tr class="text-c">
							<td><input type="checkbox" value="1" name=""></td>
							<td>${storeIn.v7SBrand }</td>				
							<td>${storeIn.v7SType }</td>				
							<td>${storeIn.v7SName }</td>				
							<td>${storeIn.v7SStandard }</td>				
							<td>${storeIn.v7SUnit }</td>				
							<td>${storeIn.v7SPrice }</td>				
							<td>${storeIn.v7SCount }</td>				
							<td>${storeIn.v7SPriceCount }</td>				
							<td>${storeIn.v7SRemarks }</td>				
							<td class="td-manage">
								<a style="text-decoration:none" title="编辑"   onclick="general_open_window_no_param('编辑','${ctx}/inProduct/gotoInProductEditView.action?id=${storeIn.id}')" id="bianji" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
								<%-- <a style="text-decoration:none" title="删除" 	  onClick="general_change_status('${storeIn.id}','3','${ctx}/inProduct/updateInProductStatus.action','确认删除该产品？','已删除！')"href="javascript:;" class="ml-5" ><i class="Hui-iconfont">&#xe6e2;</i></a> --%>
